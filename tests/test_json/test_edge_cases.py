...
def test_json_read_no_suffix(tmp_path: Path):
    file = tmp_path / "test"
...
def test_json_create_no_suffix(tmp_path: Path):
    file = tmp_path / "test"
    xjson.create(file, {"a": 1})
    assert file.with_suffix(".json").exists()


def test_json_update_create_nested(tmp_path: Path):
...
def test_json_update_dead_code(tmp_path: Path):
...
def test_json_update_type_error(tmp_path: Path):
...
def test_create_nested_list_expand():
    ...
    res = xjson._create_nested_path([], ["2", "a"], 1)  # pyright:ignore[reportArgumentType]
    assert res == [None, None, {"a": 1}]


def test_create_nested_list_existing():
    res = xjson._create_nested_path([{"a": 1}], ["0", "b"], 2)  # pyright:ignore[reportArgumentType]
    assert res == [{"a": 1, "b": 2}]
